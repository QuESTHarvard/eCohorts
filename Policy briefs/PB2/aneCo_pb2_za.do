...
*-------------------------------------------------------------------------------		
	* Number of ANC visits
		...
		replace totvisits = totvisits+ 1 
			...
			graph pie if anygap!=1, over(viscat) plabel(_all percent, format(%9.2g)) legend(off)
		...
		* Number of routine ANC or ANC referral visits  at each follow-up call
		...
		graph pie if anygap!=1, over(viscatref) plabel(_all percent, format(%9.2g)) 
*-------------------------------------------------------------------------------
	* VISIT CASCADE
*-------------------------------------------------------------------------------	
	* all eCohort women have ANC1
	...
	forval i=1/6 {
		...
		tabstat firstvisit secvisit thirdvisit if anygap!=1, stat(mean count) col(stat)
...
*-------------------------------------------------------------------------------	
	...
	* Proportion given or prescribed IFA & report taking IFA (at least once for each)
		...
		egen ifafreq=rowmax(m2_603_r*) // KE didn't assess IFA frequency. Q is "currently taking"
		...
		tab1 timelyblood dailyifa if anemia==1 & anygap!=1

		tabstat firstblood secblood thirdblood if anygap!=1, stat(mean count) col(stat)
...
*-------------------------------------------------------------------------------	
	...
	* Proportion of women who had their weight measured at least once per trimester enrolled in ANC
		...
		tabstat firstweight secweight thirdweight if anygap!=1, stat(mean count) col(stat)
...
*-------------------------------------------------------------------------------					
	* Proportion of women who got at least one urine test per trimester enrolled in ANC
		...
		tabstat firsturine securine thirdurine if anygap!=1, stat(mean count) col(stat)
...
*-------------------------------------------------------------------------------					
	* Proportion of women who got at least one ultrasound per trimester enrolled in ANC
		...
		tabstat firstus secus thirdus if anygap!=1, stat(mean count) col(stat)
...
*-------------------------------------------------------------------------------
u temp.dta, clear
	replace intraqual=intraqual*100
	encode country, g(co)
	recode co 2=3 3=2
	lab def co1 1"Ethiopia" 2"Kenya" 3"India" 4"South Africa"
	lab val co co1
	
graph box intraqual, over(co) ylabel(0(20)100, labsize(small)) ytitle("Intrapartum Care Quality Index") asyvars ///
		box(1, fcolor(navy) lcolor(navy) lwidth(thin)) marker(1, mcolor(navy)) ///
		box(2, fcolor(gold) lcolor(gold) lwidth(thin)) marker(3, mcolor(gold)) ///
		box(3, fcolor(midgreen) lcolor(midgreen) lwidth(thin)) marker(3, mcolor(midgreen)) ///
		box(4, fcolor(ebblue) lcolor(ebblue)lwidth(thin)) marker(3, mcolor(ebblue)) scheme(white_tableau)

tabstat severep , by(co) co(stat) stat(mean count)
graph bar (mean) severep, over(co) blabel(bar, format(%9.1g))  ytitle(`"Proportion of women reporting delivery complications"', size(small))
