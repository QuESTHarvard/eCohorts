...
*-------------------------------------------------------------------------------	
	...
	* Proportion of women who had their weight measured at least once per trimester enrolled in ANC
		...
		replace thirdweight = 1 if m3_412b==3 & endtrime==3
		...
		tab1 timelyweight if m1_malnutrition==1 
		...
		* Proportion of undernourished women given or prescribed food supplements at least once in pregnancy
		...
		ta foodsupp if m1_malnutrition==1
*-------------------------------------------------------------------------------
	* URINE
*-------------------------------------------------------------------------------					
...
*-------------------------------------------------------------------------------					
	* Proportion of women who got at least one ultrasound per trimester enrolled in ANC
		...
		replace firstus = 1 if m3_412f==1 & endtrime==1
		...
		replace secus = 1 if m3_412f==1 & endtrime==2
		...
		replace thirdus = 1 if m3_412f==1 & endtrime==3
...
preserve	
	keep redcap_record_id  bslga m2_ga_r* qual* 
	rename (bslga ) (m2_ga_r0 )
	
	reshape long m2_ga_r qual, i(redcap_record_id) j(round)
...
*-------------------------------------------------------------------------------
	* DELIVERY COMPLICATIONS (ICU, Blood)
*-------------------------------------------------------------------------------	
	
	recode m3_707 0/48=0 49/max=1, g(ext) 
	egen complic=rowmax(m3_706 m3_705 ext)
	
*-------------------------------------------------------------------------------		
* Equipment (Module 0)
*-------------------------------------------------------------------------------		

egen elec=rowmax(m0_206 m0_208)
egen anc_staff=rowtotal(m0_101d m0_102d m0_103d m0_104d m0_105d m0_106d m0_107d m0_108d ///
			m0_109d m0_110d m0_111d m0_112d m0_113d m0_114d m0_1a_et m0_1b_et m0_1c_et m0_1d_et m0_1e_et m0_1f_et m0_1g_et)
gen comput_inter =  m0_219==1 
	replace comput_inter= 0 if m0_220==0
	gen bp_st= m0_401a==1
/*-------------------------------------------------------------------------------
	* LINE GRAPH DANGER SIGNS
*-------------------------------------------------------------------------------					
	egen dangerm1=rowmax(m1_814a m1_814b m1_814c m1_814d m1_814e m1_814f m1_814g)
	
	forval i=1/8 {
			egen dangerm2_`i' = rowmax(m2_203a_r`i' m2_203b_r`i' m2_203c_r`i' ///
							m2_203d_r`i' m2_203e_r`i' m2_203f_r`i' m2_203g_r`i' )
		}
preserve	
	keep redcap_record_id  bslga m2_ga_r* dangerm1 dangerm2* 
	rename (bslga dangerm1) (m2_ga_r0 dangerm2_0)
	
	reshape long m2_ga_r dangerm2_, i(redcap_record_id) j(round)
	
	gen rga = round(m2_ga_r, 1.0)
	
	* Now collapse/group by gestational_age_weeks
	collapse (count) total_obs=dangerm2_ ///
         (sum) dangerm2_ , by(rga)
		 drop if rga>=41
		 drop if total_obs<16

	* Create proportion
	gen proportion = dangerm2_ / total_obs

	twoway  (lpolyci proportion rga) (scatter proportion rga), ///
	ylabel(0(.2)1) xlabel(6(4)40) xtitle("Gestational age in weeks") ///
	ytitle("Proportion reporting a danger sign") legend(off) scheme(white_tableau)
	
restore	
*-------------------------------------------------------------------------------
	* LINE GRAPH SELF REPORTED HEALTH
*-------------------------------------------------------------------------------					
	
	recode m1_201 4/5=1 1/3=0 , g(poorhealth)
	forval i=1/8 {
		recode  m2_201_r`i' 4/5=1 1/3=0 , g(poorhealth`i')
	}
preserve	
	keep redcap_record_id  bslga m2_ga_r* poorhealth* 
	rename (bslga poorhealth) (m2_ga_r0 poorhealth0)
	
	reshape long m2_ga_r poorhealth, i(redcap_record_id) j(round)
	
	gen rga = round(m2_ga_r, 1.0)
	
	* Now collapse/group by gestational_age_weeks
	collapse (count) total_obs=poorhealth ///
         (sum) poorhealth , by(rga)
		 drop if rga>=42
		 drop if total_obs<16

	* Create proportion
	gen proportion = poorhealth / total_obs

	twoway  (lpolyci proportion rga) (scatter proportion rga), ///
	ylabel(0(.2)1) xlabel(6(4)40) xtitle("Gestational age in weeks") ///
	ytitle("Proportion reporting poorhealth") legend(off) scheme(white_tableau)
...
*-------------------------------------------------------------------------------					
		recode m3_707 min/167=0 168/max=1, g(ext)
		egen bcomplic=rowmax(m3_703 m3_705 m3_706 ext)

		egen dangerbsl=rowmax(m1_814b m1_814c m1_814f m1_814g)
		
		egen bcomplic2=rowmax(m3_704*) /* 704	ASK: ALL WOMEN (INCLUDING FACILITY AND HOME BIRTHS)

Did you experience any of the following problems during your delivery, or not?
a.	Seizures
b.	Blurred vision
c.	Severe headaches
d.	Swelling in hands/feet
e.	Labor over 12 hours
